...
# Define other function for making new base call using trace columns
def new_new_call(row):
    traces = [row['Chnl9'], row['Chnl10'], row['Chnl11'], row['Chnl12']]
    traces.sort()
    if traces[-2] != 0 and traces[-1] >= traces[-2] * 2:
        if row['max_chnl'] == 'Chnl9':
...
trace_df['new_call'] = trace_df.apply(lambda row: new_call(row),
                                      axis=1)
trace_df['new_new_call'] = trace_df.apply(lambda row: new_new_call(row),
                                          axis=1)
...
# View discrepancies
diffs = np.where(trace_df['Basecall'] != trace_df['new_new_call'])
diffs_df = trace_df.iloc[diffs]

# Sum Ns
print('Genewiz', sum(trace_df['Basecall'] == 'N'))
print('Original', sum(trace_df['new_call'] == 'N'))
print('New', sum(trace_df['new_new_call'] == 'N'))
